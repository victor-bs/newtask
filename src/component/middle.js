import React from 'react';
import './middle.css';
import crypto from './cryptonew.png'
import ftx from './BannerFTX.png'

class Middle extends React.Component {
    render() {
        return (
            <div className="middle">
                <div className="middle-f">
                    <div className="values">
                        <div className="text-c">
                            <div class="text-50">0.81 %</div>
                            <div class="text-5m">5 Mins</div>
                        </div>
                        <div className="text-c">
                            <div class="text-50">1.71 %</div>
                            <div class="text-5m">1 Hour</div>
                        </div>
                        <div style={{ maxWidth: "40%" }}>
                            <div className="values-m">
                                <div class="subText">
                                    <span class="subText-h">Best Price to Trade</span>
                                </div>
                                <div class="values-h" style={{ paddingbottom: "10px" }}>₹ 45,59,240</div>
                                <div class="subText">Average BTC/INR net price including commission</div>
                            </div>
                        </div>
                        <div className="text-c">
                            <div class="text-50">8.29 %</div>
                            <div class="text-5m">1 Days</div>
                        </div>
                        <div className="text-c">
                            <div class="text-50">14 %</div>
                            <div class="text-5m">7 Days</div>
                        </div>
                    </div>
                    <a>
                        <img src={crypto} />
                    </a>
                </div>
                <div className="table" style={{ margin: "0px auto" }}>
                    <table className="table-body">
                        <thead>
                            <tr>
                                <th>
                                    <h4><span class="pointer">#</span></h4>
                                </th>
                                <th>
                                    <h4><span class="pointer">Platform</span></h4>
                                </th>
                                <th>
                                    <h4><span class="pointer">Last Traded Price</span></h4>
                                </th>
                                <th>
                                    <h4><span class="pointer">Buy / Sell Price</span></h4>
                                </th>
                                <th>
                                    <h4><span class="pointer">Difference</span></h4>
                                </th>
                                <th>
                                    <h4><span class="pointer">Savings</span></h4>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="align-middle">
                                    <h4>1</h4>
                                </td>
                                <td class="align-middle">
                                    <a>
                                        <h4>
                                            <img src="data:image/png;base64," class="exchange-logo" />
                                            <span className="exchange-name ">WazirX</span>
                                        </h4>
                                    </a>
                                </td>
                                <td class="align-middle">
                                    <h4>₹ 49,50,009</h4>
                                </td>
                                <td class="align-middle">
                                    <h4>
                                        <span>₹ 49,50,009 / ₹ 49,56,400</span>
                                    </h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">8.65 %</h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">▲ ₹ 3,93,914</h4>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                    <h4 class="table-text">2</h4>
                                </td>
                                <td class="align-middle">
                                    <a>
                                        <h4>
                                            <img src="https://hodlinfo.com/static/media/bitbns.dcee6cf4.png" className="exchange-logo" />
                                            <span class="exchange-name">Bitbns</span>
                                        </h4>
                                    </a>
                                </td>
                                <td class="align-middle">
                                    <h4>₹ 49,31,710</h4>
                                </td>
                                <td class="align-middle">
                                    <h4>
                                        <span>₹ 49,31,710 / ₹ 49,33,591</span>
                                    </h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">8.24 %</h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">▲ ₹ 3,75,615</h4>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                    <h4>3</h4>
                                </td>
                                <td class="align-middle">
                                    <a>
                                        <h4>
                                            <img src="data:image/png;base64," class="exchange-logo" />
                                            <span class="exchange-name ">Giotus</span>
                                        </h4>
                                    </a>
                                </td>
                                <td class="align-middle">
                                    <h4>₹ 49,42,000</h4>
                                </td>
                                <td class="align-middle">
                                    <h4><span>₹ 49,42,000 / ₹ 49,62,000</span></h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">8.47 %</h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">▲ ₹ 3,85,905</h4>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                    <h4>4</h4>
                                </td>
                                <td class="align-middle">
                                    <a>
                                        <h4>
                                            <img src="data:image/png;base64," class="exchange-logo" />
                                            <span class="exchange-name ">Colodax</span>
                                        </h4>
                                    </a>
                                </td>
                                <td class="align-middle">
                                    <h4>₹ 25,83,138</h4>
                                </td>
                                <td class="align-middle">
                                    <h4><span>₹ 25,75,167 / ₹ 26,08,984</span></h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-red">-43.30 %</h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-red">▼ ₹ 19,72,956</h4>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                    <h4>5</h4>
                                </td>
                                <td class="align-middle">
                                    <a>
                                        <h4>
                                            <img src="https://hodlinfo.com/static/media/zebpay.232215c8.png" class="exchange-logo" />
                                            <span class="exchange-name ">Zebpay</span>
                                        </h4>
                                    </a>
                                </td>
                                <td class="align-middle">
                                    <h4>₹ 49,66,985</h4>
                                </td>
                                <td class="align-middle">
                                    <h4>
                                        <span>₹ 49,70,000 / ₹ 49,66,985</span>
                                    </h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">9.02 %</h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">▲ ₹ 4,10,890</h4>
                                </td>
                            </tr>
                            <tr>
                                <td class="align-middle">
                                    <h4>6</h4>
                                </td>
                                <td class="align-middle">
                                    <a>
                                        <h4>
                                            <img src="data:image/png;base64," class="exchange-logo" />
                                            <span class="exchange-name ">CoinDCX</span>
                                        </h4>
                                    </a>
                                </td>
                                <td class="align-middle">
                                    <h4>₹ 49,62,723</h4>
                                </td>
                                <td class="align-middle">
                                    <h4><span>₹ 49,62,723 / ₹ 49,66,523</span></h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">8.92 %</h4>
                                </td>
                                <td class="align-middle">
                                    <h4 className="color-green">▲ ₹ 4,06,629</h4>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div class="middle-ftx" style={{ padding: "20px" }}>
                    <a>
                        <img src={ftx} style={{ width: "100%" }} />
                    </a>
                </div>
            </div>
        )
    }
}


export default Middle;